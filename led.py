...
_prev_rgb = np.tile(253, config.N_PIXELS)
...
def _update_pi():
    ...
    """
    global pixels, _prev_pixels, _prev_rgb
    # Truncate values and cast to integer
    pixels = np.clip(pixels, 0, 255).astype(int)
    ...
    diff = np.nonzero(np.subtract(rgb, _prev_rgb))
    # Update the pixels
    for i in diff[0]:
        strip.setPixelColor(int(i), int(rgb[i]))
    _prev_rgb = np.copy(rgb)
    strip.show()
    return
...
def _update_pi_test():
    ...
    for i in range(config.N_PIXELS):
        #pi = p[:,i].astype(int)
#        if p[0][i] == _prev_pixels[0][i] and \
#           p[1][i] == _prev_pixels[1][i] and \
#           p[2][i] == _prev_pixels[2][i]:
#            continue
        # Ignore pixels if they haven't changed (saves bandwidth)
#        if np.array_equal(p[:, i], _prev_pixels[:, i]):
#           continue
        rgb = (p.take(0, i) << 8) + (p.take(1, i) << 16) + p.take(2,i)        
        strip._led_data[i] = int(rgb) #int(rgb[i])
#    _prev_pixels = np.copy(p)
    strip.show()
    return
